...
public class DownloadActivity extends ListActivity {
    ...
    private void initUI() {
        ...
        btnDelete.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                // Make sure a transfer is selected
                if (checkedIndex >= 0 && checkedIndex < observers.size()) {
                    // Deletes a record and delete the file
                    File file = new File(observers.get(checkedIndex).getAbsoluteFilePath());
                    if(file.isFile()) {
                        file.delete();
                    }
    ...
    private void beginDownload(String key) {
        ...
        // file.
        Log.v("debug", "Saving to : ...." + this.getFilesDir().toString());
        File file = new File(this.getFilesDir().toString() + "/hw001/" + key);

        // Initiate the download
        TransferObserver observer = transferUtility.download("hw001", key, file);
        /*
